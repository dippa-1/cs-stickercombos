...
export const actions = {
    default: async ({ request, params }) => {
        ...
        const response = await fetch(`http://api.cs-tracker.com:8000/label/${name}`, {
            method: 'PUT',
            headers: {
...
export const load: PageServerLoad = async ({ params }) => {
  ...
  } | null = await fetch(`http://api.cs-tracker.com:8000/label/${params.name}`).then(res => res.json()).catch(e => error(500, e));
